import { INTERACTION_THRESHOLD, ANIMATION_DURATION } from './constants.js'
  ...
  create(options = {}) {
    ...
    const instance = {
      ...
      _onPointerMove(e) {
        ...
        if (delta > 0) {
          const offset = Math.min(delta, INTERACTION_THRESHOLD.DRAG_MAX_OFFSET)
      ...
      _onPointerUp(e) {
        ...
        const delta = this._pointerCurrentY - this._pointerStartY
        if (delta > INTERACTION_THRESHOLD.DRAG_CLOSE) {
          this.hide()
        } else {
      ...
      hide(result = null) {
        ...
        this._cleanupTimer = setTimeout(() => this._teardown(), ANIMATION_DURATION.BOTTOM_SHEET_CLOSE)
      }
    }
