...
| 路径 | 角色 |
|------|------|
| `index.html` | 页面入口、脚本顺序、版本展示 |
| `constants.js` | 全局常量 |
| `utils.js` | 工具、校验、`KEYS`、`LS` |
| `app.js` | 主状态、名单、任务、持久化 |
| `actions.js` | 菜单动作、导入导出、趋势等业务流程 |
| `action-views.js` | 视图工厂 |
| `modal.js` / `bottom-sheet.js` / `scorepad.js` | 组件 |
| `css/index.css` | 样式入口 |

**脚本加载顺序：**
`constants.js` → `utils.js` → `core.js` → `back-handler.js` → `modal.js` → `bottom-sheet.js` → `scorepad.js` → `app.js` → `action-views.js` → `actions.js` → `boot.js`
