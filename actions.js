...
const Actions = {
  ...
  deferFullscreenWork(root, task, delay = 140) {
    ...
    const controller = Modal ? Modal.getProgressiveController(root) : null
  ...
  togglePrivacyNames() {
    ...
    if (State) {
      ...
      Toast.show(State.prefs.privacyNames ? '已开启防泄漏姓名' : '已关闭防泄漏姓名')
    }
  },
  ...
  async cardColor() {
    ...
    const { root, preview, presetHost, picker, code } = ActionViews.createColorShell(selected)
    const apply = hex => {
      ...
      Array.from(presetHost.children).forEach(btn => btn.classList.toggle('active', btn.dataset.color === selected))
    }
    ...
    const val = await Modal.show({
      ...
      loadingMask: false,
      btns: [
        { text: '重置', val: defaults.cardDoneColor },
        { text: '取消', val: false },
  ...
  asgManage() {
    ...
    const { root, list, introHost } = ActionViews.createAsgManageShell()
    const pool = new Map()
    let mounted = new Set()
    let hero = null
    let work = null
  ...
  imp() {
    ...
    const ui = ActionViews.createImportShell()
    ...
    ui.applyBtn.addEventListener('click', () => {
      ...
      try {
        ...
        ui.applyBtn.disabled = false
        if (BottomSheet) {
          BottomSheet.alert('导入失败：' + err.message)
        } else {
  ...
  parseImportData(raw) {
    ...
    if (!State || !Validator) return null
  ...
  studentOverview() {
    ...
    const ui = ActionViews.createStudentOverviewShell()
    let chrome = null
    let work = null
    let renderToken = 0
    ...
    const mountChrome = () => {
      ...
      chrome = ActionViews.createStudentOverviewChrome()
  ...
  quizTrend() {
    ...
    const ui = ActionViews.createQuizTrendShell()
    ...
    const mountChrome = () => {
      if (chrome) return chrome
      chrome = ActionViews.createQuizTrendChrome()
      ui.heroHost.replaceChildren(chrome.hero)
      ui.toolbarHost.replaceChildren(chrome.toolbar)
      Object.assign(ui, chrome)
      return chrome
    ...
    const renderStudentCard = student => {
      ...
      if (card._trendTimelineKey !== student.timelineKey) {
        refs.chartEl.replaceChildren(ActionViews.createTrendSparkline(student.entries))
...
if (UI) {
  UI.actions = {
    has: a => typeof Actions[a] === 'function',
    run: a => Actions[a](),
...
Actions.ctx = {
  ...
  bottomSheet: globalThis.BottomSheet,
  toast: Toast,
  views: ActionViews,
  colorUtil: ColorUtil,
